...
export class CompTableService {
  ...
  async buildCompTable(options: CompTableOptions): Promise<CompTableData> {
    ...
    if (options.companies.length > 10) {
      throw new BadRequestException('Maximum 10 companies allowed');
    }
    ...
    try {
      ...
      if (rows.length === 0) {
        this.logger.warn('No data found for any of the specified companies');
        // Return empty table instead of throwing error
        return {
      ...
      this.logger.error('Error building comp table:', error.stack);

      // Handle Prisma errors
      if (error.code && error.code.startsWith('P')) {
        handlePrismaError(error, 'Comp table');
      }
      ...
      throw new BadRequestException(
        'Failed to build comparison table. Please try again.',
      );
    }
  }

  /**
   * Build a single row for a company (OPTIMIZED with batch loading)
   */
  private async buildCompTableRow(
    ...
    // OPTIMIZATION: Fetch all metrics for this company in a single query
    ...
    const metricMap = new Map(
      metricDataList.map((m) => [m.normalizedMetric, m]),
    );

    for (const metric of metrics) {
      const metricData = metricMap.get(metric);
      if (metricData) {
