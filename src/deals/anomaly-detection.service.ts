...
export class AnomalyDetectionService {
  ...
  async detectAnomalies(
    ...
    this.logger.log(`Detecting anomalies for deal ${dealId}`);
    ...
    if (!deal || !deal.ticker) {
      throw new Error('Deal not found or ticker missing');
    }
    ...
    if (!types || types.includes('statistical_outlier')) {
      anomalies.push(...(await this.detectStatisticalOutliers(deal.ticker)));
    }

    if (!types || types.includes('sequential_change')) {
      anomalies.push(...(await this.detectSequentialChanges(deal.ticker)));
    }

    if (!types || types.includes('trend_reversal')) {
      anomalies.push(...(await this.detectTrendReversals(deal.ticker)));
    }

    if (!types || types.includes('management_tone_shift')) {
      anomalies.push(...(await this.detectToneShifts(deal.ticker)));
    }
