...
export class MetricRegistryService implements OnModuleInit {
  ...
  resolve(query: string, tenantId?: string): MetricResolution {
    ...
    if (bestMatch.rating >= 0.70) {
      ...
      this.lruCache.set(cacheKey, unresolved);
      this.logger.warn(
        `Unresolved metric "${query}" (tenantId: ${tenantId || 'none'}) at ${new Date().toISOString()} — suggestions: ${uniqueSuggestions.map((s) => s.display_name).join(', ')}`,
      );
      this.logResolution(unresolved, tenantId, true);
      return unresolved;
    }
    ...
    this.logger.warn(
      `Unresolved metric "${query}" (tenantId: ${tenantId || 'none'}) at ${new Date().toISOString()} — no fuzzy matches above 0.70`,
    );
    const unresolved = this.buildUnresolved(query);
    this.lruCache.set(cacheKey, unresolved);
    this.logResolution(unresolved, tenantId, true);
    ...
    for (const { key, content } of yamlFiles) {
      try {
        ...
        for (const [canonicalId, rawDef] of Object.entries(parsed)) {
          ...
          // Derive db_column for atomic metrics
          if (metric.type === 'atomic') {
            if (metric.statement && UNIVERSAL_STATEMENTS.has(metric.statement)) {
